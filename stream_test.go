...
func TestStream_fillDataToReadBuffer(t *testing.T) {
	...
	stream, _ = client.OpenStream()
	fillDataTimes := 5
	for i := 0; i < fillDataTimes; i++ {
		data := make([]byte, size)
		rand.Read(data)
		slice = newBufferSlice(nil, data, 0, false)
		err = stream.fillDataToReadBuffer(bufferSliceWrapper{fallbackSlice: slice})
	}
	fmt.Println(len(stream.pendingData.unread))
	assert.Equal(t, fillDataTimes, len(stream.pendingData.unread))
	stream.Close()
	...
	//assert.Equal(t, 0, len(stream.pendingData.unread))
	//assert.Equal(t, nil, err)
	//Serverstream, _ := server.OpenStream()
	//Serverreader := Serverstream.BufferReader()
	//// 5.read response
	//respData, err := Serverreader.ReadBytes(size)
